...
BOOL CSerialPort::InitPort(
    CWnd* pPortOwner,      // the owner (CWnd) of the port (receives message)
    ...
    UINT writebuffersize)  // size to the writebuffer
{
  ...
  TRACE(
      "Initialisation for communicationport %d completed.\nUse Startmonitor to "
      "communicate.\n",
      portnr);
...
UINT CSerialPort::CommThread(LPVOID pParam) {
  ...
  for (;;) {
    ...
    // and an m_OverlappedStructerlapped structure specified).  This call will
    // cause the m_OverlappedStructerlapped element m_OverlappedStruct.hEvent,
    // which is part of the m_hEventArray to be placed in a non-signeled state
    // if there are no bytes available to be read, or to a signeled state if
    ...
    if (!bResult) {
      ...
      // read.  If you proceed and call ReadFile(), it will return immediatly
      // due to the async port setup, but GetOverlappedResults() will not return
      ...
      // ClearCommError(). This call will reset the event handle, and if there
      // are no bytes to read we can loop back through WaitCommEvent() again,
      // then proceed. If there are really bytes to read, do nothing and
      // proceed.
    ...
    switch (Event) {
      ...
      case 1:  // read event
      {
        ...
        if (CommEvent & EV_CTS)
          ::SendMessage(port->m_pOwner->m_hWnd, WM_COMM_CTS_DETECTED, (WPARAM)0,
        ...
        if (CommEvent & EV_BREAK)
          ::SendMessage(port->m_pOwner->m_hWnd, WM_COMM_BREAK_DETECTED,
                        (WPARAM)0, (LPARAM)port->m_nPortNr);
        if (CommEvent & EV_ERR)
          ::SendMessage(port->m_pOwner->m_hWnd, WM_COMM_ERR_DETECTED, (WPARAM)0,
                        (LPARAM)port->m_nPortNr);
...
//
DCB CSerialPort::GetDCB() { return m_dcb; }

//
// Return the communication event masks
//
DWORD CSerialPort::GetCommEvents() { return m_dwCommEvents; }

//
// Return the output buffer size
//
DWORD CSerialPort::GetWriteBufferSize() { return m_nWriteBufferSize; }
