...
bool Timer::set_loop_rate_hz(double hz) {
    ...
    _loop_duration_s = std::chrono::duration<double>{1000000./hz};
    std::cout << "[Timer] debug duration set: " << _loop_duration_s.count() << std::endl;
...
bool Timer::sleep_till_next() {
    _next_loop_start_t += std::chrono::duration_cast<std::chrono::microseconds>(_loop_duration_s);
    std::cout << "[Timer] debug target set: " <<
        std::chrono::duration_cast<std::chrono::seconds>(_next_loop_start_t - Clock::now()).count() << std::endl;
